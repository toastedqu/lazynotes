---
jupytext:
  formats: md:myst
  text_representation:
    extension: .md
    format_name: myst
kernelspec:
  display_name: Python 3
  language: python
  name: python3
---
# Reality

The ONLY reality of the observable universe is **COMPUTATION**.

&nbsp;

## Computation
### Prerequisites
- **Phenomenon** : Any mental object or factor of existence.
- **Attribute**: A perceivable, identifiable, distinguishable feature of a phenomenon.
- **State**: A set of fixed attribute values of all phenomena in an arbitrary temporal-spatial scale at an arbitrary time point.

```{note} Example
Flower is a phenomenon.

The color, shape, odor of a flower are attributes.

When a white lily blooms, its physical state consists of 6 white pedals, 6 stamens with (dark) red anther & light green filament, and 1 pistil. 
```

&nbsp;

### Definition
Computation is **state transition**.

Computation operates at any temporal-spatial scale. It can be as small as a quark, as large as the observable universe, as short as the Planck time, as long as the cosmological decade.

Computation has 3 core properties:
- **Impermanence**
    - Computation means change. Change is dynamic.
    - Every state is dying, and every state is emerging.
    - Hence, **no state represents reality**.
- **Unity**
    - Every phenomenon is dependent on each other, either directly or indirectly.
    - The observable universe is merely a united computational process.
    - No phenomenon can exist with a self identity separable from other phenomena.
    - Hence, **no phenomenon exists in reality**.    
- **Non-dualism**
    - All dualistic labels are human concepts created & reinforced by computation throughout evolution due to survival instinct.
    - Computation just is. It has no dualistic label by nature.
    - Hence, **no attribute exists in reality**.

All 3 lead to the one and only true property - **Emptiness**.
- Every concept assumes a fixed, separate self identity by definition, violating all 3 core properties of computation.
- Hence, **no concept exists in reality**.


```{note} Example
- **Impermanence**
    - A flower never "began" on its own. Sunlight pours energy into a seed. Soil, rain, air, bacteria, and a bee that pollinated some parent plant months ago all converge at one point in spacetime, and we call that convergence "a flower". It is just a node where countless threads of the computation network briefly cross. 
    - Then, a human cuts it, a florist prices it, packs it, ships it, sells it, and the instant money changes hands the flower stops being mere biology and becomes a symbol: love at a wedding, grief at a funeral, an apology, a birthday. That meaning exists nowhere in the petals. It lives only in human minds, assigned from outside. 
    - The flower sits in a vase, the assigned meaning fades as attention drifts elsewhere, the cells lose water, it wilts, decomposes, and returns to the soil, which becomes the condition for the next seed. 
    - "Birth" and "death" are just two more labels stamped onto one continuous, dependent process. Nothing was ever truly created or destroyed. The network only rearranged itself.
- **Unity**
    - Take the flower apart and ask "where is the flower?"
    - Pull off the petals, the stamen, the pistil, the sepals, the stem, the leaves. Point to any one piece: is that the flower? No. So keep dissecting. 
    - Each petal is just cells. Each cell is organelles, water, and proteins. Each protein is a chain of molecules. Each molecule is atoms. Each atom is a nucleus and electrons, then quarks and fields and probability.
    - At no layer do you ever find a hidden "flower-essence" sitting inside.
    - The "flower" was only ever a label we painted over a temporary arrangement of parts that are themselves arrangements of parts - labels all the way down, with no self-standing object at the bottom.
- **Non-dualism**
    - We slice the world of flowers into neat dualities: a red rose means passion, a white lily means death, a chrysanthemum is celebratory in one culture and funereal in another, a "weed" is just a flower someone decided was unwanted. Beautiful versus ugly, lucky versus unlucky, sacred versus ordinary - none of these pairs is found anywhere in the flower itself.
    - Scan the cells, the pigments, the wavelengths of reflected light: you will find chemistry, but not "passion", not "mourning", not "good" or "bad". The opposites live entirely on our side of the network, projected outward and then mistaken for properties of the thing.
    - And every label only has meaning by leaning on its opposite - "beautiful" is empty without "ugly", "sacred" needs "profane" to mean anything, so the dualities don't even stand on their own - they arise together, define each other, and dissolve together. Strip the projections away and there is no red-rose-passion and no white-lily-grief, no two opposing things at all - just one undivided process the network is running, which we then carve into pairs purely for our own convenience.

Hence, the concept "flower" does NOT exist in reality.

Similarly, you.
```

&nbsp;

### For You
Some preliminary concepts about humans:
- **Five Aggregates**: Form, Feeling, Perception, Mental Formation, Consciousness.
- **Six Sense Bases**: Eye, Ear, Nose, Tongue, Body, Mind.

Computation absolutely doesn't give a fuck about you. Here's why:
- **Uncertainty**
    - No phenomenon has a fixed separable self identity, so a phenomenon is always changing based on far more phenomena than anyone could observe.
    - Theoretically, at any time point, you can ALWAYS predict the next state from the current state as long as you have ALL the information. 
    - Practically, it is impossible for you to obtain all the information because all phenomena are dependent on each other. The lack of information leads to prediction errors, which quantifies the uncertainty you often ignore in various circumstances. 
    - Hence, from your perspective, **no expectation is deterministic**, which directly threatens your survival.
- **Uncontrollability**
    - No phenomenon has a fixed separable self identity, so a phenomenon is always changing based on far more phenomena than anyone could observe.
    - Your direct influence on any phenomenon is negligible on a larger temporal-spatial scale than your defined state.
    - You don't even have total control over your own Five Aggregates and Six Sense Bases.
    - More importantly, you think "you" "influence" "things", but in reality **it's just computation doing its own thing**. There's no "you". There's no "things". There's no "influence".
    - Control gives you a sense of safety, which is crucial for survival. Unfortunately, for the sake of survival, you fail to see **control is an illusion**.
- **Illusion**
    - Every state is computed by computation.
    - Every pheonomena at any given time point is computed by computation.
    - Every single attribute at any given time point is computed by computation.
    - Therefore, the current state of your Five Aggregates and Six Sense Bases is computed by computation.
    - Therefore, **your entire life experience is nothing but a MOVIE directed by computation**.
    - You are not even the actor because there is none. An "actor" phenomenon is also computed by computation.
    - When an angry Redditor shouts "Everything is BS!", they are right.
    - Every event in our life experience is essentially bullshit computed by computation.
    - **Everything is an illusion**.

> "Free will" is computed by computation. Change my mind.

## Life
### Problem
1. **Survival instinct** is the root cause of the entirety of your life experience.
    - At any given point, your brain only wants to optimize your survival capabilities because of survival instinct, even if that leads to suicide (to escape extreme pain which threathens survival) or searching for life meaning (to justify your survival in human society).
2. **Conditionality** is ingrained in your genes by your survival instinct throughout centuries of evolution.
    - You perceive different phenomena and events with different dualistic labels based on how they affect your survival.
    - Hence, your perception of life becomes purely conditioned on your experience with external phenomena.
3. **Suffering** is the rejection of reality. 
    - When you label life as "good", it's easy to accept reality because this label satisfies survival.
    - When you label life as "bad" due to your negative experience with external phenomena, you tend to reject reality because this label threatens survival.
4. **Attachment** is the root of suffering.
    - When you perceive certain external phenomena as beneficial for your survival, you form attachment, meaning you give a fuck.
    - Such attachment to "good" external phenomena has 3 forms, where the last includes the first 2:
        - **Greed**: Cling to what you want.
        - **Hatred**: Push away what you don't want.
        - **Ignorance**: Ignore reality.
    - If you cannot get what you want, if you lose what you love, if you keep getting what you don't want, if you cannot get rid of what you hate, you suffer.
    - Attachment & Conditionality assumes control, certainty, staticity, dualism, and concept existence, all of which are illusions created and reinforced by computation, originating from survival instinct.

Therefore, if you are not facing extreme threats to your physical survival, you suffer only because:
1. **You give a fuck.**
2. **You cannot see reality.**

As soon as you were born, your movie started, and you think you are the main character in your movie.

**Unfortunately, it's all just computation, but you get so absorbed into your own movie that you no longer see reality.** So you suffer.

```{note} Example
Suppose in a snippet of your movie, you applied to your dream company. You nailed every round till the final interview, and the interviewer just didn't like you for whatever unknown reason, and you got rejected. Months of preparations became obsolete. Now, what do you do? If you don't see reality, you'd probably get angry or sad or cry or curse the interviewer or facetime your family and friends to rant or whatever, or if your emotional regulation skill is strong you'd move on quickly. Doesn't matter. At this stage, you still believe you are the actor in the movie.

Let's rewind the human-specific properties of computation (i.e., reality). 
- **Illusory**: If you see reality, you already understand you are in an illusory movie directed (and acted) by computation. Getting rejected by your dream company is merely a computation result.
- **Uncontrollable**: The computation result is beyond your control because you literally have no control in reality. It is not even in the control of the final interviewer. They don't even know why they didn't like you. Computation made them dislike you at that specific time point for whatever reason, probably genetics and negative life experiences. Doesn't matter.
- **Uncertain**: You gave your best shot. You put your best possible effort to give the best possible interview performance, but personal preferences are factors that are unpredictable. The interview outcome is unpredictable from the beginning. You simply maximized your probability of getting an offer, but you can never alter reality toward a favorable outcome for you.

Now, let's rewind the core properties of computation (i.e., reality).
- **Non-dualism**: The rejection just happened. That's it. You attached a negative label to it because computation told you so. How? Computation shaped your Five Aggregates through various life events to make you perceive this company as the best possible match for you, and you perceive "not getting what you want" as negative because your survival capability is no longer optimized, all according to computation. You can also perceive the same event as positive by saying "Wow, I've learnt an incredible life lesson!" but computation didn't choose this path. You feel negative because of computation, when in reality, this event is neutral by nature. 
    - When you feel negative, the occurrence of the feeling has become part of reality as well. Therefore, you feeling negative is also neutral by nature. This reaction to a life event is called "second arrow" in Buddhism, and you can easily shoot more arrows on top of this, like "Why am I feeling negative again? I should feel positive/neutral!" Then you enter an endless spiral of thoughts and emotions.
    - When dissected this way, you might find it funny and easily manageable. However, this happens way more often than people think. We will address the solution later.
- **Unity / Emptiness**: If you curse the interviewer, you'd understand where they come from - family stress, job stress, demographic bias, bad mood, or they really just hate you personally, whatever. They didn't choose this. Computation did. You happen to be affected by this computation result. That's it.
    - Compassion & Forgiveness is a critical topic in Buddhism, but I won't force it. You don't need to forgive them for ruining your life. Understanding reality alone would alleviate your suffering.
- **Impermanence**: This too shall pass. In fact, this event has already passed. You holding the event in your mind is the reason why you think this has not passed yet.

So what is the reality in this specific case? "You got rejected by the final interviewer." Period. Nothing more. Nothing less. If you add anything to it - interpretation, analysis, ration, emotion, or whatever - the occurrence of these things is real, but the content of these things is illusory.
```

&nbsp;

### Solution

> Just see reality.

That's it. Just see reality.

As soon as you see reality, you are at peace.

Let me stop you right here.
1. Did your mind react to this sentence negatively? Finding me arrogant and ignorant? Finding this sentence absurd and funny? Thinking it's so much easier said than done? Thinking I'm ignoring all the people who suffer from severe mental issues? Thinking of the meme "just be happy"?
2. Watch it closely. Who's saying all these shits in your mind? Did "you" "choose" to react this way? Or did these thoughts just emerge naturally as a reaction? If so where did they emerge from?
3. How do the thoughts sound like? Did you paint any mental image inside your mind? If so how does it look like?
4. How do the thoughts feel like? Did you get angry? If so how angry? Do you feel it anywhere in your body? How does it feel like physically?
5. Are you still feeling that thought? Has your feeling changed a bit as you are reading this text?
6. **This, is reality.**

As soon as you see reality, you are at peace.

&nbsp;

Let me use "pain" to represent all negative experiences in the mind.

**"Peace" does NOT mean "painless".** Absolutely not. Here's why:
1. Your mind is a bullshit machine fostered by computation through centuries of evolution based on survival instinct. 
2. When you feel pain, you feel pain. The pain is here. This is reality. 
3. Whatever bullshit your mind adds on top of it, is also reality.
4. If you overthink a lot, whatever bullshit your mind adds on top of the already generated bullshits, is also reality.
5. "Peace" means, "**The pain is here. Ok. My mind is adding these and those bullshits on top of it. Ok.**"
6. Watch the difference - **The occurrence of the bullshits IS real. The content of the bullshits IS NOT real**, because it is infected with human concepts, which are intrinsically illusory and nonexistent.
    - When you give a fuck about the content of the bullshits, you trap yourself in illusions.
    - However, if you just give a fuck about the occurrence of the bullshits, you see reality.

As soon as you see reality, you are at peace.

&nbsp;

*"Then what?"* **Then you move on.**

*"What about all the interpretations & analyses? What about all the imagined scenarios? What about all these emotional residues? What about the endless spiral of bullshits?"* **Yea they are here.**

*"That's it?"* **Yea.**

*"Why shouldn't I give a fuck about them?"* **You already did.**

*"But I've already wasted so much time and energy on them..."* **Yea that happened.**

*"But"* **Move on.**

*"I'm moving on but they keep coming back, this time even stronger!"* **Yea they are here.**

*"I reacted with strong emotions to them again and again and again!"* **Yea that happened.**

*"WHAT?"* **Yea. Move on.**

*"I got rejected by my dream school. I feel awful. I hate myself. I regret not spending more time preparing for SAT..."* **Yea that happened. Yea the regret is here.**

*"My crush told me to fuck off. I cried... I still want to cry... I am crying..."* **Yea that happened. Yea the crying is here.**

*"I got fired from my company for some random fucking bullshit that had nothing to do with me! THIS WORLD IS FUCKING UNFAIR! THIS UNIVERSE FUCKING HATES ME!"* **Yea that happened. Yea the anger is here.**

&nbsp;

**No. Don't push them away. Let them be.**

&nbsp;

We can have these conversations all day. What you need to understand is, 

> Whatever happened happened.
>
> Whatever happens happens.
>
> Whatever will happen will happen.
>
> There is ONLY computation.

This applies to the occurrences of all your thoughts and feelings too.

As soon as you see reality, you are at peace.

&nbsp;

### Practice
**The only way to practice seeing reality, is to see reality**, anytime, anywhere, as often as you can, as long as you can, even when you have an endless spiral of thoughts and emotions.

Fuck all conventional shits - meditation, exercise, healthy diets, cold showers, sleep, reading, journaling, supplements, ... - fuck all of them. 

You cannot cure the cause by treating the symptoms.

If you remember to see reality, pause, slow, relax, watch. Things happened. Things are here. Computation. Move on.

If you don't often remember to see reality, **practice by facing difficulty**.

Do things you think are difficult. Form doesn't matter. Time doesn't matter. Location doesn't matter. Mental state doesn't matter. Computation makes your mind tell you they matter. They don't. 

Here's why it works - You experience the highest variety of extreme negative emotions when you do difficult things.

Remember, the goal is to enhance your skill of seeing reality, so keep seeing reality while doing them.

Find your difficulty level. Play. See reality. Level up. Next level. Play. See reality. Level up. Next level. If you fail, retry. If you fail, retry. Follow computation. Let computation do the work.

Conventional shits work because they are perceived as difficult. Conventional shits don't work because they are perceived as difficult. It's never about the method. 

**It's about you being trapped in illusions created by computation**.



&nbsp;

### Finale
What do you seek? Resilience? Strength? Discipline? Diligence? Integrity? Equanimity? Compassion? Courage? ...?

You already are.

You are computation. You are reality.

&nbsp;

If you want to do something, see the want, feel the want, and do it.

If your mind strongly objects, see the emotions, feel the emotions, and do it.

If external phenomena impede you from doing it, see the obstacles, feel the obstacles, and do it.

If you fail, see the failure, feel the failure.

If there's another chance, see the chance, feel the chance, and do it.

If there isn't, see the impossibility, feel the impossibility, and move on.

&nbsp;

Computing.